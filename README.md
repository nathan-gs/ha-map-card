...
| `gradualOpacity`      |                                       | The total amount of opacity will be gradually distributed across the paths. `gradualOpacity` is divided by the number of paths to calculate the opacity step for each path. The base opacity is generated by (1 - `gradualOpacity`), and as each path progresses, its opacity gradually darkens by the step amount.|
